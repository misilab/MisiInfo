...
let frManual = Manual(h1: "Manuel utilisateur — Français", sections: [
    ...
    Section("Préréglages de conformité (v1.4)", body: "Dans la section Conformité, sélectionnez un préréglage parmi sept normes pros embarquées : Netflix 4K HDR10, Netflix 4K SDR, Broadcast EBU R128 (HD TV), DCP Cinéma (DCI), YouTube 4K, ProRes 422 HQ Delivery, Web HTML5. MisiInfo valide automatiquement chaque règle (codec, résolution, profondeur, primaries, transfer, sample rate, LUFS, True Peak…) avec un statut vert/orange/rouge, un score global sur 100, et une explication pédagogique pour chaque non-conformité. Idéal pour un workflow de QC avant livraison."),
    Section("Mode comparaison (v1.4)", body: "Importez au moins deux fichiers, puis dans la barre d'outils ouvrez le menu Comparer et activez le mode. Sélectionnez le fichier B à comparer, et MisiInfo affiche les deux analyses côte à côte avec les différences surlignées en orange (icône ⚠). Pratique pour vérifier qu'un export correspond bien à un master, ou comparer deux profils d'encodage."),
    Section("CLI standalone (v1.4)", body: "MisiInfo livre un binaire en ligne de commande, parfait pour les pipelines DIT et la post-production automatisée. Utilisation : `misiinfo video.mp4` (sortie texte), `misiinfo --json clip.mov > rapport.json` (sortie JSON structurée), `misiinfo --check broadcast.ebu.r128 livraison.wav` (validation conformité). Codes retour : 0 OK, 1 erreur d'analyse, 2 non-conformité critique, 64 mauvais arguments. Installation : `sudo cp scripts/misiinfo /usr/local/bin/`."),
...
let enManual = Manual(h1: "User Manual — English", sections: [
    ...
    Section("Compliance presets (v1.4)", body: "In the Compliance section, pick one of seven embedded professional standards: Netflix 4K HDR10, Netflix 4K SDR, Broadcast EBU R128 (HD TV), DCP Cinema (DCI), YouTube 4K, ProRes 422 HQ Delivery, Web HTML5. MisiInfo automatically validates every rule (codec, resolution, bit depth, primaries, transfer, sample rate, LUFS, True Peak…) with a green/orange/red status, an overall score out of 100, and a pedagogical explanation for each non-compliance. Perfect for a QC workflow before delivery."),
    Section("Comparison mode (v1.4)", body: "Import at least two files, then open the Compare menu in the toolbar and enable the mode. Choose file B to compare against, and MisiInfo displays both analyses side by side with differences highlighted in orange (⚠ icon). Useful to verify that an export matches a master, or to compare two encoding profiles."),
    Section("Standalone CLI (v1.4)", body: "MisiInfo ships a command-line binary, perfect for DIT pipelines and automated post-production. Usage: `misiinfo video.mp4` (plain text), `misiinfo --json clip.mov > report.json` (structured JSON), `misiinfo --check broadcast.ebu.r128 delivery.wav` (compliance validation). Exit codes: 0 OK, 1 analysis error, 2 critical non-compliance, 64 bad arguments. Installation: `sudo cp scripts/misiinfo /usr/local/bin/`."),
...
let esManual = Manual(h1: "Manual de usuario — Español", sections: [
    ...
    Section("Preajustes de conformidad (v1.4)", body: "En la sección Conformidad, elija uno de los siete estándares profesionales integrados: Netflix 4K HDR10, Netflix 4K SDR, Broadcast EBU R128 (HD TV), DCP Cine (DCI), YouTube 4K, ProRes 422 HQ Delivery, Web HTML5. MisiInfo valida automáticamente cada regla (códec, resolución, profundidad, primarios, transferencia, frecuencia de muestreo, LUFS, True Peak…) con un estado verde/naranja/rojo, una puntuación global sobre 100 y una explicación pedagógica para cada no conformidad. Ideal para un flujo de QC antes de la entrega."),
    Section("Modo comparación (v1.4)", body: "Importe al menos dos archivos, luego abra el menú Comparar en la barra de herramientas y active el modo. Seleccione el archivo B a comparar y MisiInfo mostrará ambos análisis uno al lado del otro con las diferencias resaltadas en naranja (icono ⚠). Útil para verificar que una exportación coincide con un máster, o para comparar dos perfiles de codificación."),
    Section("CLI independiente (v1.4)", body: "MisiInfo incluye un binario de línea de comandos, perfecto para pipelines DIT y postproducción automatizada. Uso: `misiinfo video.mp4` (texto plano), `misiinfo --json clip.mov > informe.json` (JSON estructurado), `misiinfo --check broadcast.ebu.r128 entrega.wav` (validación de conformidad). Códigos de salida: 0 OK, 1 error de análisis, 2 no conformidad crítica, 64 argumentos incorrectos. Instalación: `sudo cp scripts/misiinfo /usr/local/bin/`."),
