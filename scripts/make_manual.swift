...
let frManual = Manual(h1: "Manuel utilisateur — Français", sections: [
    ...
    Section("Export du rapport", body: "Trois boutons dans l'en-tête du détail permettent de : (1) révéler le fichier source dans le Finder, (2) copier le rapport complet dans le presse-papier, (3) exporter le rapport en .txt à l'emplacement de votre choix."),
    ...
    Section("Mises à jour automatiques", body: "MisiInfo vérifie automatiquement la disponibilité d'une nouvelle version sur GitHub à chaque ouverture. Vous pouvez aussi déclencher la vérification manuellement depuis le menu MisiInfo → Vérifier les mises à jour, ou le bouton circulaire dans la barre d'outils."),
...
let enManual = Manual(h1: "User Manual — English", sections: [
    ...
    Section("Exporting the report", body: "Three buttons in the detail header let you: (1) reveal the source file in Finder, (2) copy the full report to the clipboard, (3) export the report as a .txt file to a location of your choice."),
    ...
    Section("Automatic updates", body: "MisiInfo automatically checks for new releases on GitHub every time the app is opened. You can also trigger a check manually from MisiInfo → Check for Updates or the circular button in the toolbar."),
...
let esManual = Manual(h1: "Manual de usuario — Español", sections: [
    ...
    Section("Exportación del informe", body: "Tres botones en la cabecera del detalle permiten: (1) mostrar el archivo de origen en el Finder, (2) copiar el informe completo al portapapeles, (3) exportar el informe como archivo .txt en la ubicación que elija."),
    ...
    Section("Actualizaciones automáticas", body: "MisiInfo verifica automáticamente la disponibilidad de nuevas versiones en GitHub cada vez que se abre la aplicación. También puede iniciar una verificación manualmente desde MisiInfo → Buscar actualizaciones o el botón circular de la barra de herramientas."),
